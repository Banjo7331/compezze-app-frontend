...
// ADRES GATEWAYA + PREFIX QUIZU
const QUIZ_WS_URL = 'ws://localhost:8000/quiz/ws'; 

// Definicja generycznego typu payloadu
...
class QuizSocketClient extends BaseSocketClient<QuizSocketPayload> {
  ...
  // Subskrypcja pokoju Quizowego
  public subscribeToRoomUpdates(roomId: string, callback: (data: QuizSocketPayload) => void): string {
    // WAŻNE: Backend (QuizEventWebSocketNotifier) wysyła na "/topic/quiz/{roomId}"
    // (W Survey było /topic/room, tu zmieniliśmy na /topic/quiz dla porządku w backendzie)
