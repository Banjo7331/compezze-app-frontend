...
import { 
  ...
  FormControlLabel, Switch, Grid, Paper, // Dodaj CircularProgress jeśli chcesz ikone ładowania
} from '@mui/material';
...
// Feature Imports
// WAŻNE: Importujemy QuestionTypeValues, aby móc używać wartości w JSX i logice
import type { QuestionType, CreateSurveyFormRequest } from '../model/types';
...
export const SurveyCreateForm: React.FC<SurveyCreateFormProps> = ({ onCancel, onSuccess }) => {
  ...
  const [questions, setQuestions] = useState<QuestionDraft[]>([
    // Używamy QuestionTypeValues.SINGLE_CHOICE zamiast QuestionType.SINGLE_CHOICE
    { id: Date.now(), title: '', type: QuestionTypeValues.SINGLE_CHOICE, possibleChoices: [''] }
  ]);
  const [isSubmitting, setIsSubmitting] = useState(false);

  // --- Validation Logic (bez zmian) ---
  const TITLE_MIN = 8;
  const TITLE_MAX = 20;
  ...
  const areQuestionsValid = questions.every(q => {
    ...
    // FIX: Użycie QuestionTypeValues.TEXT do porównania
  ...
  const addQuestion = () => {
    ...
    setQuestions([
      ...questions, 
      // Używamy QuestionTypeValues.SINGLE_CHOICE
      { id: Date.now(), title: '', type: QuestionTypeValues.SINGLE_CHOICE, possibleChoices: [''] }
    ]);
  };
  ...
  const updateQuestion = (id: number, field: keyof QuestionDraft, value: any) => {
    setQuestions(questions.map(q => {
      if (q.id === id) {
        // FIX: Użycie QuestionTypeValues.TEXT
  ...
  // (addOption, updateOption, removeOption - bez zmian)
                                        ...
                                        <Grid container spacing={2}>
                                            ...
                                            <Grid size={{ xs: 12, md: 4 }}>
                                                <FormControl fullWidth>
                                                    ...
                                                    >
                                                        {/* FIX: Użycie QuestionTypeValues do wartości */}
